...
func createOutlineItemDict(ctx *Context, bms []Bookmark, parent *IndirectRef, parentPageNr *int) (*IndirectRef, *IndirectRef, int, error) {
	...
	for i, bm := range bms {
		...
		/* PIROGOM
		   Why should an error be returned if the current index's
		   PageFrom is less than the previous index's PageFrom? block it first*/
...
*	RemoveBookmarks
**/
func (ctx *Context) RemoveBookmarks() error {
	rootDict, err := ctx.Catalog()
	if err != nil {
		return err
	}
